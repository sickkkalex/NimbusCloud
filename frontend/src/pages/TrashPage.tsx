...
import { getTrash, type SearchResult } from '../api/search'
...
export default function TrashPage() {
  ...
  const fetchTrash = async (isRefresh = false) => {
    ...
    try {
      ...
      setError('Impossibile caricare il cestino.')
    } finally {
      setLoading(false)
      setRefreshing(false)
    }
    ...
    <div className="min-h-screen bg-gray-50/50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pb-24 md:pb-8">
        ...
        >
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Cestino</h1>
            <p className="text-gray-500">I file eliminati rimarranno qui per 30 giorni</p>
          </div>
        </motion.div>
        ...
        >
          ...
          {error && (
            <div className="card p-8 text-center">
              ...
              <button onClick={() => fetchTrash()} className="btn-secondary text-sm mt-2">Riprova</button>
            </div>
          )}
          ...
          {!loading && !error && result.files.length === 0 && result.folders.length === 0 && (
            <div className="card p-16 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-800 mb-2">Il cestino è vuoto</h3>
              <p className="text-gray-400">Nessun file o cartella è stato eliminato di recente.</p>
            </div>
          )}
            ...
            <AnimatePresence>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {result.files.map((file, index) => (
                  <FileCard key={file.id} file={file} index={index} onRefresh={fetchTrash} />
                ))}
              </div>
            </AnimatePresence>
          )}
